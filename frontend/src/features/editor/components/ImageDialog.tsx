  ...
  return (
    <Dialog open={open} onOpenChange={next => { onOpenChange(next); if (!next) setUrl(''); }}>
      <DialogContent className="sm:max-w-md dialog-dark">
        <DialogHeader>
          <DialogTitle className="text-white/90 text-sm">Insert an image</DialogTitle>
        ...
        <div className="space-y-3 mt-1">
          <div>
            <label className="text-[11px] text-white/50 mb-1 block">From your device</label>
            <input
              type="file"
              accept="image/*"
          ...
          <div>
            <label className="text-[11px] text-white/50 mb-1 block">Or a URL</label>
        ...
        <DialogFooter className="gap-2 mt-2">
          <Button variant="outline" onClick={() => onOpenChange(false)} className="glass-btn border-white/10 text-xs h-8">
            Cancel
          </Button>
          <Button
            onClick={() => { onInsert(url.trim()); onOpenChange(false); setUrl(''); }}
            disabled={!url.trim()}
            className="text-black font-semibold text-xs h-8"
            style={{ background: 'linear-gradient(135deg, var(--accent-500), var(--accent-300))' }}
          >
            Insert
          </Button>
        </DialogFooter>
